...
const DispatchManagement: React.FC = () => {
  ...
  const [currentRecord, setCurrentRecord] = useState<DispatchTask | null>(null);
   ...
   * 打开编辑Modal
   */
  const handleEdit = (record: DispatchTask) => {
    setModalMode('edit');
    ...
    try {
      ...
      if (modalMode === 'create') {
        await http.post('/api/v1/dispatch/tasks', {
          ...values,
          scheduledStart: values.scheduledStart ? values.scheduledStart.format() : undefined,
          scheduledEnd: values.scheduledEnd ? values.scheduledEnd.format() : undefined,
        });
   ...
   * 处理删除
   */
  const handleDelete = async (record: DispatchTask) => {
    try {
